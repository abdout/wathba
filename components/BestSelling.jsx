...
const BestSelling = ({ dict, lang }) => {
    ...
    return (
        <div className='px-6 my-30 max-w-6xl mx-auto'>
            <Title title={dict?.products?.bestSelling || 'Best Selling'} description={loading ? dict?.common?.loading || 'Loading...' : `${dict?.products?.showing || 'Showing'} ${products.length < displayQuantity ? products.length : displayQuantity} ${dict?.products?.of || 'of'} ${products.length} ${dict?.products?.products || 'products'}`} href={`/${lang}/shop`} dict={dict} lang={lang} />
