    ...
    extraReducers: (builder) => {
        ...
        builder
            ...
            .addCase(syncCart.fulfilled, (state, action) => {
                ...
                if (action.payload) {
                    // Extract cart from payload (API returns { cart, message } for some endpoints)
                    const cart = action.payload.cart || action.payload;
                    // Ensure cart is an object and not a string or other type
                    if (typeof cart === 'object' && cart !== null && !Array.isArray(cart)) {
                        state.cartItems = cart;
                        // Calculate total safely, ensuring only numbers are summed
                        state.total = Object.values(cart)
        ...
        builder
            ...
            .addCase(fetchCart.fulfilled, (state, action) => {
                ...
                state.lastSynced = new Date().toISOString();
                if (action.payload) {
                    // For fetchCart, payload might be { cart, items, total, itemCount }
                    const cart = action.payload.cart || action.payload;
                    // Ensure cart is an object and not a string or other type
                    if (typeof cart === 'object' && cart !== null && !Array.isArray(cart)) {
                        state.cartItems = cart;
        ...
        builder
            .addCase(updateCartItem.fulfilled, (state, action) => {
                if (action.payload) {
                    // Extract cart from payload (API returns { cart, message })
                    const cart = action.payload.cart || action.payload;
                    // Ensure cart is an object and not a string or other type
                    if (typeof cart === 'object' && cart !== null && !Array.isArray(cart)) {
                        state.cartItems = cart;
